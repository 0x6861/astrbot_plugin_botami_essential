...
- `/今天吃什么`：从当前群的食物库中均匀随机推荐一项。
- `/今天吃什么 add <食物1> [食物2 ...]`：按输入顺序批量添加食物。
- `/今天吃什么 rm <食物1> [食物2 ...]`：批量删除食物。
- `/今天吃什么 list`：按添加顺序列出当前群的食物。

食物名称包含空格时需要使用引号，例如
`/今天吃什么 add "番茄牛腩" "牛肉 拉面"`。名称会去除首尾空白并按原文保存，
重复判断不区分大小写；每项长度为 1 至 64 个字符，不能包含换行符或制表符。
每个群最多保存 200 项食物，群内成员均可添加和删除，私聊不处理该命令。

批量添加和删除采用整批提交：只要存在重复、非法名称、数量超限或待删除项不存在，
整批操作都会被拒绝，不会产生部分修改。食物库初始为空，不提供内置默认项；
数据按群隔离，并使用版本化 JSON 保存到插件数据目录的
`food_recommender/foods.json`。
...
Minecraft 领域逻辑位于 `src/minecraft`，食物推荐逻辑位于
`src/food_recommender`，共享的
